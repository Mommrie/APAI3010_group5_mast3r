...
def main_demo(tmpdirname, model, retrieval_model, device, image_size, server_name, server_port, silent=False,
    ...
    # 导入视频处理模块（如果添加到不同路径，请相应调整）
    # ----- 新增：视频处理模块导入 -----
    try:
        from dust3r.utils.video import process_video_for_reconstruction
    except ImportError:
        # 如果导入失败，提供友好的错误信息
        def process_video_for_reconstruction(*args, **kwargs):
            raise ImportError("视频处理模块未找到，请确保已创建 dust3r/utils/video.py 文件")
    # ----- 视频处理模块导入结束 -----
    ...
    with get_context(gradio_delete_cache) as demo:
        ...
        with gradio.Column():
            ...
            def process_video(video_path, max_frames, method, interval=15):
                ...
                try:
                    ...
                    if not silent:
                        ...
                        print(f"提取方法: {method}, 最大帧数: {max_frames}")
                    ...
                    if not silent:
                        print(f"准备传递 {len(file_components)} 个文件到图像处理流程")
                        # 打印几个示例路径
                        if len(file_components) > 0:
                            print(f"样例文件: {file_components[0]}")
            ...
            def log_inputfiles(files):
                ...
                try:
                    ...
                    if not isinstance(files, list):
                        print(f"文件输入非列表类型: {type(files)}")
                        return files
            
                    print(f"收到 {len(files)} 个文件")
                    if len(files) > 0:
                        print(f"第一个文件类型: {type(files[0])}")
                        if isinstance(files[0], tuple) and len(files[0]) >= 1:
                            print(f"样例文件路径: {files[0][0]}")
                except Exception as e:
                    print(f"记录文件信息时出错: {e}")
                return files
            ...
            inputfiles.change(
                log_inputfiles,
                inputs=[inputfiles],
                outputs=[inputfiles]  # 用同一个组件作为输出只是为了保持流程继续
            )
            # ----- 视频处理功能结束 -----
