...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
                            clean_depth=False, transparent_cams=False, cam_size=0.05, do_sr=False):
    """
    ...
    if do_sr:
        ...
        pts = np.concatenate([p[m] for p,m in zip(pts3d, msk)], axis=0)
        ...
        mesh_o3d, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
            pc, depth=9, scale=1.1)
        dens = np.asarray(densities)
        keep = dens > np.quantile(dens, 0.01)
        ...
        outpath = os.path.join(outdir, 'surface_reconstruction.glb')
        tri.export(outpath)
        if not silent: print(f'(exported surface mesh to {outpath})')
        outfile = outpath

    else:
        # ——— “正常” 点云或网格 导出 —— #
        # 这里调用原来的 helper，把点云或合并网格转成 glb
        outfile = _convert_scene_output_to_glb(
            outdir, scene.imgs, pts3d, msk,
            scene.get_focals(), scene.get_im_poses(),
            cam_size=cam_size,
            cam_color=None if transparent_cams else CAM_COLORS,
...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
                            ...
                            scenegraph_type, winsize, refid, do_sr):
    """
    ...
    outfile = get_3D_model_from_scene(outdir, silent, scene, min_conf_thr, as_pointcloud, mask_sky,
                                      clean_depth, transparent_cams, cam_size, do_sr)
    ...
    with gradio.Blocks(css=""".gradio-container {margin: 0 !important; min-width: 100%};""", title="DUSt3R Demo") as demo:
        ...
        with gradio.Column():
            ...
            with gradio.Row():
                ...
                do_sr = gradio.Checkbox(value=False, label="Surface Reconstruction (Poisson)")
